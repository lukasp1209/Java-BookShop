...
2. Next, we want to react to the change event of the sorting dropdown box:
   ...
   ```
3. Let's add the selected option's (=`event.target`) value  to the browsers search parameters:
   ```javascript
   const searchParams = new URLSearchParams(location.search)
   const sorting = event.target.value
   searchParams.set('sort', sorting)
   ```
4. If the user selects the `DEFAULT` option, we additionally want to move to the first page:
   ```javascript
   if (sorting === 'DEFAULT')
     searchParams.set('page', "1")
   ```
5. Last step - change the browsers URL:
   ```javascript
   location.search = searchParams.toString()
   ```
6. Congratulations! Now it's time to thoroughly test the sorting in your browser!<br/>
